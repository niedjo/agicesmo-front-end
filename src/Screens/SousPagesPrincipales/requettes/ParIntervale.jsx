...
export default function ParIntervale(props) {
    ...
    function jours_entre(date1, date2) {
        ...
        const difference_jours = Math.ceil(difference_ms / UN_JOUR); // Convertir en jours et retourner
    ...
    const [DateSearch1, setDateSearch1] = useState(new Date().toLocaleDateString())

    const recherche = useRef()
    const daterecherche = useRef()
    const daterecherche1 = useRef()
    ...
    function OnRequestConnexionClose (props) { 
        ...
        props.row1.forEach((r, key) => {
          ...
          const ladate = new Date(r.date_du_jour)
          if (recherche.indexOf(props.search.toLowerCase()) === -1) {
            return
          }
          if (jours_entre(ladate, new Date(DateSearch)) >= 0 && jours_entre(new Date(DateSearch1), ladate) >= 0) {
        //     return
        //   }
            Row1.push(
        ...
        props.row2.forEach((r, key) => {
          const ladate = new Date(r.date_exam)
        ...
        props.row3.forEach((r, key) => {
          const ladate = new Date(r.date_examen)
        ...
        //   }
            Row3.push(
                <tr align="center" key={key}>
                    <td>{r.nom}</td>
                    <td>{new Date(r.date_examen).toLocaleDateString()}</td>
                    <td>{r.type_fiche_imagerie}</td>
                    <td>{r.conclusion}</td>
        ...
        props.row4.forEach((r, key) => {
          const ladate = new Date(r.date_achat)
        ...
        props.row6.forEach((r, key) => {
          const ladate = new Date(r.date_de_dette)
        ...
        props.row8.forEach((r, key) => {
            const ladate = new Date(r.date_consultation)
        ...
        props.row10.forEach((r, key) => {
            const ladate = new Date(r.date_administration)
    ...
    const setOnRequestConnexionClose = () => {
        ...
        setDateSearch1(daterecherche1.current.value)
    }
    ...
    return (
        <div className="Requettes">
            <div style={{display : "flex", alignItems : "center", backgroundColor : "#7ba2db"}}>
                ...
                <h4>Requettres par intervalle</h4>
            </div>
            <center style={{backgroundColor : "#7ba2db", paddingTop : 25, paddingBottom : 30, paddingLeft : 60, paddingRight : 60}}>
                <table>
                    <tbody>
                        <tr>
                            <td className="col-md-1"><input type="date" className="form-control" ref={daterecherche}/></td>
                            <td className="col-md-2">Date du premier jour</td>
                            <td className="col-md-1"><input type="date" className="form-control" ref={daterecherche1}/></td>
                            <td className="col-md-2">Date du dernier jour</td>
                            <td className="col-md-3"><input type="text" placeholder="Objet de la recherche" className="form-control" ref={recherche}/></td>
                            <td className="col-md-1"><button className="btn btn-primary" style={{background : "#054bb3", border : "2px solid blue"}} onClick={setOnRequestConnexionClose}>Rechercher</button></td>
                        </tr>
                    </tbody>
                </table>
            </center>
